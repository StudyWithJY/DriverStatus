...
def get_eyes_mouth(file):
    ...
    for index, eye_roi in enumerate(left_eyes):
        cv2.imwrite(f'cropped_left_eye_{index + 1}.png', eye_roi)

    for index, eye_roi in enumerate(right_eyes):
        cv2.imwrite(f'cropped_right_eye_{index + 1}.png', eye_roi)
...
for img_file in os.listdir(directory):
    ...
    for face in faces:
        ...
        n = []
        k = get_eyes_mouth(img)
        n.append(ImageLabelPair(left_eye_opened, k[0], right_eye_opened, k[1], mouth_opened, k[2]))
